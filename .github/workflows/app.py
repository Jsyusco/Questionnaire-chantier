...
# -----------------------------------------------------------
# --- NOUVELLE LOGIQUE D'ATTENTE DE PHOTOS (AJOUT) ---
# -----------------------------------------------------------

# Dictionnaire : "Nom de la Section" : ["Colonne1 à additionner", "Colonne2 à additionner", ...]
# Assurez-vous que "Nom de la Section" correspond EXACTEMENT à ce qui est écrit dans votre colonne 'section' de formsquestions
SECTION_PHOTO_RULES = {
    "Bornes DC": ['R [Plan de Déploiement]', 'UR [Plan de Déploiement]'],
    # Ajoutez d'autres sections ici si nécessaire. Par exemple :
...
def get_expected_photo_count(section_name, project_data):
    ...
    for col in columns:
        ...
        # Nettoyage de la valeur (gestion des vides, strings, etc.)
        try:
            if pd.isna(val) or val == "":
                num = 0
            else:
                num = int(float(str(val).replace(',', '.'))) # Gère "2.0" ou "2,0"
        except Exception:
            num = 0
        
        total_expected += num
        # On garde le nom court pour l'affichage (ex: R [Plan...] devient R)
    ...
    if expected_total is not None and expected_total > 0:
        ...
        for _, row in section_rows.iterrows():
            if str(row['type']).strip().lower() == 'photo':
                # S'il y a une question photo, on active le flag
                photo_questions_found = True
                q_id = int(row['id'])
                val = answers.get(q_id)
                # La réponse est une liste si c'est un file_uploader multiple
                if isinstance(val, list):
        ...
        if photo_questions_found and current_photo_count != expected_total:
            missing.append(
                f"⚠️ <b>Nombre de photos incorrect pour '{section_name}'</b>.<br>"
                f"Attendu : <b>{expected_total}</b> (calculé : {detail_str})<br>"
...
if st.session_state['step'] == 'PROJECT_LOAD':
    ...
    if st.session_state['step'] == 'LOOP_DECISION':
        ...
        if not st.session_state['current_phase_name']:
            ...
            # ------------------------------------------------------------------
            # --- AFFICHAGE DE L'ATTENTE PHOTO (AJOUT) ---
            # ------------------------------------------------------------------
            ...
            with c2:
                if st.button("💾 Valider la phase"):
                    ...
                    if is_valid:
                        ...
                        # L'affichage utilise les tags HTML pour les sauts de ligne si nécessaire
